    ...
    const features = [
        {
            ...
            description: "Auto-adjust offers to drive traffic during low demand hours.",
        },
        {
            icon: "/images/AI2.svg",
        ...
        <section className="bg-themecolor text-white py-16 px-4">
            ...
            <div className="max-w-7xl mx-auto mt-10 grid gap-8 md:grid-cols-2 lg:grid-cols-4 pt-6">
                {features.map((item, index) => (
                    <div key={index} className="flex flex-col items-center md:items-start text-center md:text-left gap-2">
                        <Image src={item.icon} alt="AI tools" width={40} height={40} className="mb-2" />
                        <p className="font-semibold text-white font-poppins">{item.title}</p>
                        <p className="text-sm text-white/80 font-poppins">{item.description}</p>
                    </div>
                ))}
            </div>
        </section>
