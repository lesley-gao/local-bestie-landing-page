...
export default function Navbar({ config }: NavbarProps) {
  ...
  const handleCTAClick = (e: React.MouseEvent) => {
    if (config.ctaButton.useModal && config.modalConfig) {
      e.preventDefault();
      setIsModalOpen(true);
    }
    //TODO: if do not use modal, then redirect to the link
  };
            ...
            <div className="space-y-3 pt-3">
              ...
              {config.ctaButton.useModal ? (
                <button
                  ...
                  className="block w-full bg-themecolor text-white font-semibold py-3 px-4 rounded text-center clickeffect hover:bg-[#92253C] transition"
                >
                  {config.ctaButton.text}
                </button>
              ) : (
